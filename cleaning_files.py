...
def cleaner(x, y, z):
    ...
    file_path = Path('C:\\users\\brian\\documents\\school_work\\Spring_Semester_2022\\CSCI_483\\')
    backup_path = Path('C:\\users\\brian\\documents\\school_work\\Spring_Semester_2022\\CSCI_483\\test_files\\input_test_current.csv')
    # location of primary test DB
    output_path = Path('C:\\users\\brian\\.Neo4jDesktop\\relate-data\\dbmss\\dbms-0da3383b-8af6-419d-a1c7-81e6e0cfc06a\\import\\input_file_current.csv')
    # location of secondary test DB
    secondary_path = Path('C:\\users\\brian\\.Neo4jDesktop\\relate-data\\dbmss\\dbms-b7533d89-3200-4234-8e44-4a2cabb9a43c\\import\\input_file_current.csv')
    if os.getcwd() == file_path:
        print("Already there!")
        imported = pd.read_excel(x)
    else:
    ...
    print("file imported.")
    headers(input_file)
    dateCol(input_file, z)
    charCheck(input_file)
    accountNum(input_file, y, z)
    print("file cleaned.")
    input_file.to_csv(backup_path)      # creating backup of file
    input_file.to_csv(secondary_path)      # creating csv in the Neo4j import folder
    print("file saved and backed up.")
